...
os.environ["MKL_NUM_THREADS"] = "1"  # Limits MKL (Intel's Math Kernel Library) threads
os.environ["VECLIB_MAXIMUM_THREADS"] = "1"  # Limits VecLib (used by macOS Accelerate framework) threads
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_level_0", type=str, required=True, default="classic_mil")
    ...
    preproc_conf = preproc_conf[config_level_0]
    ...
    if norm == 'nonorm':
        ...
        patch_dir = preproc_conf["patch_dir_macenko_new_bracs"]
        dest_dir = preproc_conf["uni_finetuned_emb_dir_macenko_new_bracs"]
    else:
    ...
    loaded = torch.load(preproc_conf.uni_finetuned_embedder_weights, map_location="cpu")
            ...
            try:
                ...
                with h5py.File(f, 'r') as h5_file:
                    ...
                    for i in range(num_parts):
                        ...
                        imgs_tensor = torch.zeros((imgs.shape[0], imgs.shape[3], 224, 224), dtype=torch.float32) # regardless of the input patch size, the transform always brings to (3,224,224)
                    ...
                    features_all = np.concatenate(features_all).astype(np.float16)
...
# nohup 
# python generate_uni_embeddings.py --config_level_0  classic_mil_on_embeddings_bag --config_level_1 nightingale_224_224_patches --maxnum_threads 1 --num 0 --norm macenko
# > logs/generate_uni_224_224_patches_embeddings_numthread_0.txt &