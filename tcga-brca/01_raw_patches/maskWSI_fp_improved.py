...
class MaskWSI_fp:
    ...
    def calc_mask_improved(self, patch_size, mask_extrapolate=False, ADAPT=-1, verbose=0):
        ...
        print("Image at mask level after crop: ", img_openslide_np.shape)
        image_mask_level_crop = img_openslide_np
        ...
        if (perc_limits != -1).sum():
            ...
            return mask, width, height, image_mask_level_crop, scale_factor

        else:
            return np.argwhere( np.isclose(np.ones((current_image.shape[0], current_image.shape[1])), 1.) ) , width, height, image_mask_level_crop, scale_factor
